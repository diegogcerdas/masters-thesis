...
class StableDiffusion:
    ...
    def __init__(self, batch_size: int, device="cpu"):
        ...
        self.unet = UNet2DConditionModel.from_pretrained("CompVis/stable-diffusion-v1-4", subfolder="unet", torch_dtype=self.dtype).to(self.device)
        self.vae = AutoencoderKL.from_pretrained("CompVis/stable-diffusion-v1-4", subfolder="vae", torch_dtype=self.dtype).to(self.device)
        ...
        self.clip_vision = CLIPVisionModel.from_pretrained("openai/clip-vit-large-patch14", torch_dtype=self.dtype).to(self.device)
    ...
    def text_emb_to_img(self, text_embedding, return_pil, initial_latents=None, g=7.5, seed=0, steps=500):
        """
        ...
        initial_latents = torch.randn(self.latent_shape) if initial_latents is None else torch.clone(initial_latents)
        ...
        for i, ts in enumerate(tqdm(self.scheduler.timesteps)):
            ...
            if i < steps-1:
                with torch.no_grad():
                    u, t = self.unet(inp, ts, encoder_hidden_states=text_embedding).sample.chunk(2)
            else:
                u, t = self.unet(inp, ts, encoder_hidden_states=text_embedding).sample.chunk(2)
