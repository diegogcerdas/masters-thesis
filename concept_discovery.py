...
from concept_discovery_utils import ComposableDataset, freeze_params
...
def run(
    ...
    if args_add_weight_per_score:
        ...
        concept_weights = (
            torch.tensor([args_init_weight] * num_tokens)
            .reshape(1, -1, 1, 1, 1)
            .float()
        )
    ...
    for epoch in tqdm(range(args_num_train_epochs)):
        ...
        for batch in train_dataloader:
            pixel_value, input_ids, weight_id = (
                batch["pixel_values"],
                batch["input_ids"],
                batch["gt_weight_id"],
            )
            ...
            input_ids_list = [
                y.squeeze(dim=1)
                for y in input_ids.chunk(chunks=input_ids.shape[1], dim=1)
            ]
            ...
            for input_ids in input_ids_list:
                ...
                cond_scores.append(
                    unet(noisy_latents, timesteps, encoder_hidden_state).sample
                )
            ...
            uncond_encoder_hidden_states = text_encoder(uncond_text_ids)[0].repeat(
                bsz, 1, 1
            )
            uncond_score = unet(
            ...
            composed_score = uncond_score + torch.sum(
                weights.to(latents.device) * (cond_scores - uncond_score[:, None]),
                dim=1,
            )
