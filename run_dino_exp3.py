...
def main(cfg):
    ...
    print('##############################')
    print(f'### Subject {cfg.subject} Category {cfg.category} ####')
    print('##############################')
    ...
    ckpt_path1 = os.path.join(cfg.ckpt_dir, 'dino_vit', f'0{cfg.subject}_all_l_all_0')
    ckpt_path1 = os.path.join(ckpt_path1, sorted(list(os.listdir(ckpt_path1)))[-1])

    ckpt_path2 = os.path.join(cfg.ckpt_dir, 'dino_vit', f'0{cfg.subject}_all_r_all_0')
    ...
    subfolders = sorted([f for f in os.listdir(folder_all) if os.path.isdir(os.path.join(folder_all, f))])

    for subfolder in tqdm(subfolders, total=len(subfolders)):

        f1 = os.path.join(folder_all, subfolder, f'{cfg.subject}_dino_vit_preds_left.npy')
        f2 = os.path.join(folder_all, subfolder, f'{cfg.subject}_dino_vit_preds_right.npy')

        if os.path.exists(f1) and os.path.exists(f2):
            continue

        img_list = np.array([f for f in os.listdir(os.path.join(folder_all, subfolder)) if f.endswith('.png')])
        img_list_order = np.argsort([int(f.replace('.png', '')) for f in img_list])
        img_list = [os.path.join(folder_all, subfolder, f) for f in img_list[img_list_order]]
