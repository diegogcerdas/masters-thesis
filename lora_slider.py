...
from lora_slider_utils import (LoRANetwork, PromptEmbedsPair, encode_prompts,
...
def run(
    ...
    with torch.no_grad():
        prompt_pair = PromptEmbedsPair(
            ...
            positive=encode_prompts(
                tokenizer, text_encoder, [prompt_settings.positive]
            ),
            unconditional=encode_prompts(
                tokenizer, text_encoder, [prompt_settings.neutral]
            ),
            neutral=encode_prompts(
                tokenizer, text_encoder, [prompt_settings.unconditional]
            ),
    ...
    for i in pbar:
        with torch.no_grad():
            noise_scheduler.set_timesteps(
                config.train.max_denoising_steps, device=args_device
            )
            optimizer.zero_grad()
