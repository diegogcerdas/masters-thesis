...
def get_control(config, control_range, image_dim):
    control = torch.from_numpy(np.load(f'{config["measures"]}.npy')) * 10
...
if __name__ == "__main__":
    ...
    for measure in ['warmth', 'saturation', 'brightness', 'entropy', "depth", "gaussian_curvature"]:

        config = {

            'dataset_root': '/gpfs/work5/0/gusr53691/data/NSD/',
            'measures': measure,
            'patches_shape': (64, 64),
            'img_shape': (448, 448),

            'model_path': 'runwayml/stable-diffusion-v1-5',
            'prompt': 'a doodle of a house',
            'output_dir': f'./data/readout_guidance/{measure}/examples',
            ...
            'rg_kwargs': [
                {
                    'head_type': 'spatial',
                    'loss_rescale': 1,
                    'aggregation_kwargs': {'aggregation_ckpt': f'./data/readout_guidance/{measure}/checkpoints/last.pt'},
                }
            ],
