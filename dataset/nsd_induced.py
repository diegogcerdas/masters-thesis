...
class NSDInducedDataset(data.Dataset):
    ...
    def compute_targets(self, distance_matrix):
        folder = os.path.join(self.nsd.root, f"subj{self.nsd.subject:02d}", "training_split", 'targets')
    ...
    def compute_low_dim(self, distance_matrix):
        tsne = manifold.TSNE(n_components=2, metric="precomputed", init="random", random_state=self.seed)
